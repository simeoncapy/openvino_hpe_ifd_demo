...
const std::string NEP_TOPIC             = "user_emotions";
const std::string NEP_DATA              = "userEmotions";
...
int main(int argc, char *argv[]) {
    try {
        ...
        cv::Mat temp_frame; 
        cap >> temp_frame;
        modifyImage(temp_frame, frame, flipVideo);
        // ---end
        ...
        while (true) {
            ...
            temp_frame = cv::Mat();
            if (!isLastFrame) {
                frameReadStatus = cap.read(temp_frame);
            ...
            if(faces.size() != 0) // sending data via NEP
            {
                json msg;
                std::string emo = getEmotion(faceSave->getEmotions()); // get the emotion with the biggest proba NEP
                std::cout << "NEP sent emo: " << emo << std::endl;

                msg[constantes["NEP_DATA"]] = emo;
                // Sending to NEP
                pub.publish(msg);
            }
